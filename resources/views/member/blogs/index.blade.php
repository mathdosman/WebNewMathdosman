<x-app-layout>
    ...
    <x-slot name="headerRight">
        <form action="{{ route('member.blogs.index') }}" method="get">
            <x-text-input id="search" name="search" type="text" class="p-1 m-0 md:w-72 w-80 mt-3 md:mt-0" value="{{ request('search') }}" placeholder="masukan kata kunci ..." />
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="border px-6 py-4 text-center">
                                    ...
                                    <a target="_blank" href="{{ route('blog-detail',['slug'=>$value->slug]) }}" class="text-blue-600 hover:text-blue-400 px-2">lihat</a>
                                    <form action="{{ route('member.blogs.destroy',['post'=>$value->id]) }}" class="inline" onsubmit="return confirm('Yakin akan menghapus?')" method="post">
                                        @csrf
                                        @method('delete')
